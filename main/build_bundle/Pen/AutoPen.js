...
export class AutoPenCls {
    ...
    Inequalities({ items = [], ticks = [], }) {
        ...
        const writtenNum = [];
        function inequality(index, { position, sign, num, vertical }) {
            ...
            let B = [align, base];
            let T = [align, base + aHeight];
            let E = [greater ? width : 0, base + aHeight];
            if (vertical) {
                pen.set.dash(10);
                pen.set.alpha(0.5);
        ...
        let cutting = Sort(0, ...items.map((x, i) => x.position ?? defaultPosition(i)), 1);
        if (len === 2 && typeof ticks === 'string') {
            ticks = solveCompoundInequality(items[0].sign, Number(items[0].num), items[1].sign, Number(items[1].num), ticks);
        }
     ...
     * ```
     */
    TrigSolution({ trig = 'sin', k = 0, scale = 1.5, ratio = 0.7, }) {
     ...
     * ```
     */
    LinearProgram({ constraints = [], field = [0, 0, 0], contours = [], labelConstraints = [], highlights = [], ranges = [
        [-10, 10],
        [-10, 10],
    ], resolution = 0.2, grid = 0, subGrid = 0, tick = 0, showLine = true, showShade = true, showVertex = false, showVertexCoordinates = false, showVertexLabel = false, showVertexMax = false, showVertexMin = false, showIntegral = false, showIntegralLabel = false, showIntegralMax = false, showIntegralMin = false, contourColor = 'grey', constraintColors = [], }) {
        function fieldAt(p) {
            const [a, b, c] = field;
            const [x, y] = p;
     ...
     * ```
     */
    PieChart({ items, labels, angles, angleLabels, size = 2, }) {
        let O = [0, 0];
        const pen = new Pen();
    ...
    FreqPolygon(config) {
        ...
        let { pen, drawBars, drawLine, drawXTicks, drawItems, drawBounds } = HeightChart({
            items: [
                bin[0].mark - width,
                ..._.map(bin, 'mark'),
                bin.at(-1).mark + width,
...
function HeightChart({ items, freqs, boundLabels, xLabel = '', yLabel = '', grid = [5, 1], colWidth = 1.2, barWidthRatio = 1, }) {
    ...
    function drawBars() {
        function bar(x, h) {
            pen.polyshape([x - barWidth / 2, 0], [x - barWidth / 2, h], [x + barWidth / 2, h], [x + barWidth / 2, 0]);
        }
    ...
    function drawLine() {
        let points = freqs.map((v, i) => [getX(i), v]);
