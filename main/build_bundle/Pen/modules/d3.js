export class PenD3 {
    ...
    envelope(lowerBase, upperBase) {
        const LB = lowerBase;
        const UB = upperBase;
        let isPolar = (A, O, B) => AnglePolar(this.cv.pj(A), this.cv.pj(O), this.cv.pj(B)) < 180
            ? 1
            : -1;
     ...
     * ```
     */
    solid(lowerBase, upperBase, { showUpper = true, showLower = true, shadeLower = !true, shadeUpper = !true, lowerZ = 0, height = !true, envelopeOnly, } = {}) {
        ...
        if (isPoint3Ds(lowerBase)) {
            ...
            throw Error('lowerBase must be Point3D[] | [center: Point3D, radius: number]');
        }
        if (isPoint3Ds(upperBase)) {
            ...
            throw Error('upperBase must be Point3D[] | Point3D | [center: Point3D, radius: number] | [scale: number, vertex: Point3D] | number');
        }
        if (showLower)
            this.pen.polygon(...LB);
        if (showUpper)
            this.pen.polygon(...UB);
        if (shadeLower)
            this.pen.polyshade(...LB);
        if (shadeUpper)
            this.pen.polyshade(...UB);
        envelopeOnly ??=
