...
function isValidVariable(v) {
    ...
    if (Array.isArray(v))
        return v.every(isValidVariable);
...
function printThing(v) {
    if (Array.isArray(v))
        return '[' + v.map(printThing).join(',') + ']';
    if (typeof v === 'object' && v !== null)
        return ('{' +
            Object.entries(v)
...
class ErrorLogger {
    ...
    add(e) {
        this.pile.push(e instanceof Error
            ? '[' + e.name + ']\n' + e.message
            : '[Error]\n' + JSON.stringify(e));
    }
    readHtml(delimiter) {
...
export class Soil {
    ...
    runPopulate() {
        while (this.counter <= 1000) {
            ...
            try {
                ...
                if (!ok)
                    throw new Error('runPopulate: Dict Check Failed\n' + dictReport);
                if (!this.isValidated())
    ...
    runShuffle() {
        let { qn, sol, ans, hasDuplicatedOptions } = shuffleOptions(this.qn, this.sol, this.config.answer, this.config.shuffle);
