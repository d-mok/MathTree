 ...
 * ```
 */
export function StemAndLeaf({ data, labels, stem = '(tens)', leaf = '(units)', }) {
    let T = '';
    T += '\\begin{array}{r|l}';
    T += `\\text{Stem} & \\text{Leaf} \\\\ `;
 ...
 * ```
 */
export function FreqTable({ data, dataLabel, freqLabel, min = Math.min(...data), max = Math.max(...data), }) {
 ...
 * ```
 */
export function PairTable({ rowTitle, colTitle, rows, cols, cell, }) {
    function parseCell(cell) {
        if (typeof cell === 'number')
            return String(cell);
        return cell.startsWith('$') ? cell.substring(1) : `\\text{${cell}}`;
 ...
 * ```
 */
export function ShortDivision({ numbers, mode = 'HCF', }) {
