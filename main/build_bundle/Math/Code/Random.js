 ...
 * ```
 */
export function RndNs(min, max, n = 10, sort = 'none') {
 ...
 * ```
 */
export function RndRs(min, max, n = 10) {
 ...
 * ```
 */
export function RndQ(largest = 9, range) {
 ...
 * ```
 */
export function RndQs(largest = 9, range, n = 10) {
 ...
 * ```
 */
export function RndT(trueProb = 0.5) {
 ...
 * ```
 */
export function RndZs(min, max, n = 10, sort = 'none') {
    n = Math.min(Math.floor(max - min + 1), n);
    let nums = dice(() => RndN(min, max))
        .unique()
        .rolls(n)
 ...
 * ```
 */
export function RndOdd(min, max) {
 ...
 * ```
 */
export function RndEven(min, max) {
    min = Math.ceil(min / 2);
 ...
 * ```
 */
export function RndComposite(factors, n) {
    let num = 1;
    for (let i = 1; i <= n; i++) {
        num *= RndPick(...factors);
    }
 ...
 * ```
 */
export function RndPyth(max = 100) {
    let arr = [];
    for (let m = 1; m < 10; m++) {
        for (let n = 1; n < m; n++) {
            for (let k = 1; k < 10; k++) {
 ...
 * ```
 */
export function RndPoint(xRange, yRange = xRange) {
 ...
 * ```
 */
export function RndPoints(xRange, yRange = xRange, n = 10) {
    return dice(() => RndPoint(xRange, yRange))
        .unique(([x, y]) => x)
        .unique(([x, y]) => y)
        .coherent($ => $.combinations(3).every(([A, B, C]) => Slope(A, B) !== Slope(B, C)))
        .rolls(n);
}
/**
 ...
 * ```
 */
export function RndAngles(n, separation) {
 ...
 * ```
 */
export function RndOnCircle(n, separation) {
    let t = RndN(0, 360);
 ...
 * ```
 */
export function RndConvexPolygon(n, center, radius, separation) {
    let [h, k] = center;
    let r = radius;
 ...
 * ```
 */
export function RndData(min, max, n) {
 ...
 * ```
 */
export function RndTriangle(xRange, yRange, { minAngle = 0, maxAngle = 180, minLength = 0, obtuse = false } = {}) {
    let [x1, x2] = xRange;
    let [y1, y2] = yRange;
    let arr = [];
    for (let i = x1; i <= x2; i++) {
 ...
 * ```
 */
export function RndTrigValue(func, angle) {
    let trig = (funcName, q) => {
        if (funcName === 'sin')
            return sin(q);
        if (funcName === 'cos')
 ...
 * ```
 */
export function RndTrigEqv(result, label) {
 ...
 * ```
 */
export function RndRatio(min, max, n = 10) {
 ...
 * ```
 */
export function RndPartition(n, length, allowZero = false) {
