 ...
 * ```
 */
export function VecAdd3D(...vectors) {
 ...
 * ```
 */
export function Slide3D(A, B, ratio) {
    let r = ratio;
    let s = 1 - r;
 ...
 * ```
 */
export function PdFoot3D(point, base) {
    if (base.length === 3) {
        let [A, B, C] = base;
        return VecAdd3D(vec.projectOnPlane(vec.fromTo(A, point), vec.fromTo(A, B), vec.fromTo(B, C)), A);
    }
    else if (base.length === 2) {
        let [A, B] = base;
        return VecAdd3D(vec.projection(vec.fromTo(A, point), vec.fromTo(A, B)), A);
    }
 ...
 * ```
 */
export function Embed(plane2D, origin, xVec, yVec) {
    return plane2D
 ...
 * ```
 */
export function EmbedX(plane2D, x = 0) {
 ...
 * ```
 */
export function EmbedY(plane2D, y = 0) {
 ...
 * ```
 */
export function EmbedZ(plane2D, z = 0) {
 ...
 * ```
 */
export function FlatZ(points, z = 0) {
 ...
 * ```
 */
export function Extrude(lowerBase, upperBase, scale) {
    function padTail(arr, len) {
