 ...
 * ```
 */
export function ASequence(a, d, n = 10) {
    let arr = [];
    for (let i = 1; i <= n; i++) {
 ...
 * ```
 */
export function GSequence(a, r, n = 10) {
    let arr = [];
    for (let i = 1; i <= n; i++) {
 ...
 * ```
 */
export function QuadraticSequence(a, p, q, n) {
    let c = a;
    for (let i = 2; i <= n; i++) {
        c += p * (i - 1) + q;
    }
 ...
 * ```
 */
export function LucasSequence(first, second, p, q, n) {
    if (n === 1)
        return first;
    if (n === 2)
        return second;
