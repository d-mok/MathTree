 ...
 * ```
 */
export function ASTC(quadrant, func) {
    if (quadrant == 'I')
        quadrant = 1;
    if (quadrant == 'II')
        quadrant = 2;
 ...
 * ```
 */
export function TrigSolve(func, k) {
    if (func == 'sin') {
        if (k > 1 || k < -1)
            return [];
        if (k == 0)
 ...
 * ```
 */
export function PolarDiff(angle1, angle2) {
 ...
 * ```
 */
export function WholeBearing(polarAngle) {
    let q = polarAngle;
    q = PolarReduce(q);
    q = q.blur();
    q = q <= 90 ? 90 - q : 450 - q;
 ...
 * ```
 */
export function CompassBearing(polarAngle) {
    let q = polarAngle;
    q = PolarReduce(q);
    q = q.blur();
    if (q === 0)
