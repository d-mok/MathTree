...
import { rndSignedPoint } from './random.js'

export function Bisection(eq: zeroFunction, ranges: [number, number][]) {
    if (!ranges.some(([min, max]) => max > min)) {
        throw '[bisection] all variables are locked already'
    }

    let a: number[] = rndSignedPoint(eq, ranges, 1) // positive point
    let b: number[] = rndSignedPoint(eq, ranges, -1) // negative point
