...
import { blur, toFraction, isRational } from '../math/cal'

/**
 * A subclass of array. Designed for number array.
 */
export class Numbers extends List<number>{
     ...
     * ```
     */
    public add(nums: number | number[]): this {
     ...
     * ```
     */
    public minus(nums: number | number[]): this {
        if (!Array.isArray(nums))
            nums = Array(this.length).fill(nums)
        let zipped = this.zip(nums, (a, b) => a - b)
        return this.create(zipped)
     ...
     * ```
     */
    public times(nums: number | number[]): this {
        if (!Array.isArray(nums))
            nums = Array(this.length).fill(nums)
        let zipped = this.zip(nums, (a, b) => a * b)
        return this.create(zipped)
     ...
     * ```
     */
    public divide(nums: number | number[]): this {
        if (!Array.isArray(nums))
            nums = Array(this.length).fill(nums)
        let zipped = this.zip(nums, (a, b) => a / b)
        return this.create(zipped)
     ...
     * ```
     */
    public toPower(indices: number | number[]): this {
     ...
     * ```
     */
    public rootNth(nths: number | number[]): this {
     ...
     * ```
     */
    public toFraction(): List<[number, number]> {
     ...
     * ```
     */
    public gapsMod(mod: number): this {
     ...
     */
    public hcf(): number {
        if (this.some($ => !Number.isInteger($))) return NaN
     ...
     * ```
     */
    public reduceRatio(): this {
        if (this.except([0]).length === 0) return this.clone()
        if (this.some($ => !Number.isInteger($))) return this.clone()
        let HCF = this.hcf()
    ...
    public ratio(): this {
        if (this.except([0]).length === 0) return this.clone()
        if (this.some($ => !isRational($))) return this.clone()
    ...
    public ratioFactor(): number {
        if (this.except([0]).length === 0) return NaN
        if (this.some($ => !isRational($))) return NaN
...
declare module "./numbers" {
    ...
    namespace Numbers {
        export function of<T>(...items: T[]): Numbers & List<T>
    }
}
 ...
 * ```
 */
export function numbers(...elements: number[]): Numbers {
 ...
 * ```
 */
export function toNumbers(elements: number[]): Numbers {
