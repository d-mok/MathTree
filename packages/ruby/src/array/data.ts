...
export class Data extends Numbers {
     ...
     * ```
     */
    public modes(Nth: number = 1): this {
        if (this.length === 0) return this.create([])
     ...
     * ```
     */
    public mode(Nth: number = 1): number {
        if (this.length === 0) return NaN
     ...
     * ```
     */
    public isSingleMode(Nth: number = 1): boolean {
        const modes = this.modes(Nth)
...
declare module "./data" {
    ...
    namespace Data {
        export function of<T>(...items: T[]): Data & List<T>
    }
}
 ...
 * ```
 */
export function data(...elements: number[]): Data {
    let dt = new Data()
    dt.push(...elements)
    return dt
}
 ...
 * ```
 */
export function toData(elements: number[]): Data {
