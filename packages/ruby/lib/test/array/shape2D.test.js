...
describe("isConvex", () => {
    it("return boolean", () => {
        ...
        expect(shape2D([-6, -2], [3, 5], [3, -6], [-3, 4], [7, -1]).isConvex()).toBeTrue();
        expect(shape2D([0, 0], [3, 0], [1, 1], [0, 3]).isConvex()).toBeFalse();
        expect(shape2D([-6, -2], [3, 5], [3, -6], [-1, 1], [7, -1]).isConvex()).toBeFalse();
    });
    it_preserve_original(($) => $.isConvex());
});
describe("isConvex", () => {
    it("return boolean", () => {
        expect(shape2D([0, 0], [1, 0], [0, 1], [3, 4]).erect([1, 0, 0], [0, 1, 0])).toStrictEqual(shape3D([0, 0, 0], [1, 0, 0], [0, 1, 0], [3, 4, 0]));
        expect(shape2D([0, 0], [1, 0], [0, 1], [3, 4]).erect([0, 1, 0], [0, 0, 2])).toStrictEqual(shape3D([0, 0, 0], [0, 1, 0], [0, 0, 2], [0, 3, 8]));
    });
